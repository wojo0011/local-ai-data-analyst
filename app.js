import { histogram, cosineSimilarity } from './lib/stats.js?v=20260817-3';
import { buildColumnDescription, inferDatasetQuestions } from './lib/schema.js?v=20260817-3';
...
async function askAnalyst(){if(!state.columns.length){$('analyst-answer').textContent='Load a dataset first.';return}const question=$('analyst-question').value.trim()||state.questions.analyst[0]||'What are the strongest signals in this dataset, and what should I query next?';$('analyst-answer').textContent='Preparing deterministic evidence in DuckDB…';try{const context=await buildEvidenceContext();$('analyst-answer').textContent='Evidence ready. Loading the local model…';await loadLLM();if(!state.llm)return;$('analyst-answer').textContent='Analyzing local evidence…';const response=await state.llm.chat.completions.create({messages:[{role:'system',content:'You are a careful local data analyst. Use only the evidence provided. Never invent unavailable numbers. Separate observed evidence from hypotheses. Correlation is not causation. Recommend reproducible SQL or statistical checks for claims. Keep the response concise and educational.'},{role:'user',content:`${context}\n\nQuestion: ${question}`}],temperature:.2,max_tokens:700});$('analyst-answer').textContent=response.choices?.[0]?.message?.content||'No response returned.'}catch(error){console.error(error);$('analyst-answer').textContent=`Local analyst error: ${error.message}`}}
